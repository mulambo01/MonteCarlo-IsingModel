...
mtx crystalmatrix(int nrows, int ncols, float value){
 mtx matrix;
 int j;
 matrix.nrows=nrows;
 matrix.ncols=ncols;
 matrix.data=(float **)malloc(nrows*sizeof(float *));
 for(int i=0; i<nrows; i++){
  matrix.data[i]=(float *)malloc(ncols*sizeof(float));
  for(j=0; j<ncols; j++){
...
void mtxsave(char *filename, mtx matrix){
 ...
 for(int i=0; i<matrix.nrows; i++){
  for(j=0; j<matrix.ncols; j++){
   fprintf(file, "%.20f", matrix.data[i][j]);
...
mtx mtxsum(mtx matrix1, mtx matrix2){
 ...
 float **answ=(float **)malloc(matrix1.nrows*sizeof(float *));
 int j;
 for(int i=0; i<matrix1.nrows; i++){
  answ[i]=(float *)malloc(matrix1.ncols*sizeof(float));
...
mtx mtxmult(mtx matrix, float q){
 mtx mult;
 mult.nrows=matrix.nrows;
 mult.ncols=matrix.ncols;
 float **answ=(float **)malloc(matrix.nrows*sizeof(float *));
 int j;
 for(int i=0; i<matrix.nrows; i++){
  answ[i]=(float *)malloc(matrix.ncols*sizeof(float));
  for(j=0;j<matrix.ncols;j++){
...
mtx mtxprod(mtx matrix1, mtx matrix2){
 ...
 float **answ=(float **)malloc(prod.nrows*sizeof(float *));
 int j,k;
 for(int i=0; i<prod.nrows; i++){
  answ[i]=(float *)malloc(prod.ncols*sizeof(float));
...
mtx transpose(mtx matrix){
 ...
 float **trans=(float **)malloc(answ.nrows*sizeof(float *));
 int i,j;
 for(i=0; i<matrix.ncols; i++){
  trans[i]=(float *)malloc(answ.ncols*sizeof(float));
...
mtx mtxclone(mtx A){
 ...
 nrows=ans.nrows;
 ncols=ans.ncols;
 float **A2=(float **)malloc(nrows*sizeof(float *));
 for(int i=0; i<nrows; i++){
  A2[i]=(float *)malloc(ncols*sizeof(float));
  for(j=0; j<ncols; j++){
   A2[i][j]=A.data[i][j];
  }
 }
...
float vectprod(mtx vec1, mtx vec2){
 float result=0.0;
 int j;
...
void mtxcopy(mtx *matrix1, mtx matrix2){
 ...
 if(matrix1->nrows != matrix2.nrows){
  matrix1->data=(float **)realloc(matrix1->data, matrix2.nrows*sizeof(float *));
 ...
 for(int i=0; i<matrix2.nrows; i++){
  if(difsize){
   matrix1->data[i]=(float *)realloc(matrix1->data[i], matrix2.ncols*sizeof(float));
  }
